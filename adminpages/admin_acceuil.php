...
$query = "SELECT  count(cne) nombre_etud from etudiant ;";
  try {
...
// --------------------------------
// La requete (exemple) : tous les "objet", classés par "id".
$query = "SELECT  count(cin) nombre_prof from professeur ;";
  try {
	$pdo_select = $pdo->prepare($query);
	$pdo_select->execute();
	$NbreData = $pdo_select->rowCount();	// nombre d'enregistrements (lignes)
	$rowAll1 = $pdo_select->fetchAll();
  } catch (PDOException $e){ echo 'Erreur SQL : '. $e->getMessage().'<br/>'; die(); }
// --------------------------------
// affichage
if ($NbreData != 0) 
...
<div class="wrapper">
            ...
            <div class="sidebar" data-color="green" data-image="img/sidebar-5.jpg">
                ...
                <div class="sidebar-wrapper">
                    <div class="logo">
                        <a href="admin_acceuil.php" class="simple-text">
...
?>
		...
		<tr>
			...
			<td><?php echo $row['nombre_etud']; ?></td>
